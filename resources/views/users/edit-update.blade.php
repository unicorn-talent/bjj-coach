          ...
          <div class="card mb-4">
            ...
            <div class="card-footer bg-white border-0 pt-0">
              <div class="justify-content-between align-items-center">
				  <div class="form-group display-none" id="divvideocategories" >
					  <div class="input-group mb-2">
						 <select required name="categories_id" id="categories_id" class="form-control custom-select" >
							
						   @foreach (DB::table('videocategories')->where('mode','on')->orderBy('name')->get() as $category)
							 <option value="{{$category->id}}">{{ Lang::has('categories.' . $category->slug) ? __('categories.' . $category->slug) : $category->name }}</option>
						   @endforeach
						 </select>
					  </div>
					</div><!-- End form-group -->
                ...
                @if (auth()->user()->subscriptionsActive() && $settings->users_can_edit_post == 'on'
                  ...
                  <button type="button" class="btn e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill btn-upload btn-tooltip" data-toggle="tooltip" data-placement="top" title="{{$data->image == '' && $data->video == '' && $data->music == '' ? trans('general.upload_media') : trans('general.replace_media')}}" onclick="$('#filePhoto').trigger('click')">
                    <i class="feather icon-image f-size-25"></i>
                  </button>
                  
                  <button type="button" id="btnvideocategories" class="btn btn-upload btn-tooltip e-none align-bottom @if (auth()->user()->dark_mode == 'off') text-primary @else text-white @endif rounded-pill" data-toggle="tooltip" data-placement="top" title="{{trans('general.set_category_for_video_post')}}">
					  <i class="fa fa-list f-size-25"></i>
				  </button>
