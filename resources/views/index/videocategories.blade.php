    ...
    <div class="container">
      <div class="row justify-content-center text-center mb-sm">
        <div class="col-lg-12 py-5">
          ...
          <p class="lead text-muted mt-0">({{$users_category_count}}) {{trans_choice('users.creators_in_this_category',$users_category_count )}}</p>
        </div>
      </div>
      ...
      @include('includes.listing-videocategories')

      <div class="row">
        @if ($users_category_count != 0)

          
			  @foreach (DB::table('updates')->where('video','<>','')->where('video_categories',$titleid)->get() as $category)
				<div class="col-md-4 mb-4">
					<div class="card card-updates h-100">
						<video src='https://bjjcoach-media.s3.us-east-2.amazonaws.com/uploads/updates/videos/{{ $category->video }}' controls></video>
					</div>
				</div><!-- end col-md-4 -->
			  @endforeach
