...
class HomeController extends Controller
{
  ...
  public function __construct(Request $request, AdminSettings $settings) {
    ...
    public function videocategory($slug, $type = false)
    {
//die("test");
      $category = Videocategories::where('slug', '=', $slug)->where('mode','on')->firstOrFail();
      $title    = \Lang::has('categories.' . $category->slug) ? __('categories.' . $category->slug) : $category->name;
		$titleid = $category->id;
      switch ($type) {
        case 'new':
      ...
      if ($type == 'free') {
    	...
    	$users_category_count = Updates::distinct()
				   ->where('video_categories', '<>', null )
				   ->where('video_categories', '<>','' )
				   ->where('video_categories',  $titleid)
				   ->count('user_id');
				   
        return view('index.videocategories', [
          'users' => $users,
            'title' => $title,
            'slug' => $slug,
            'image' => $category->image,
            ...
            'description' => $category->description,
            'titleid' => $titleid,
            'users_category_count' => $users_category_count
        ]);
    }
