...
class UserController extends Controller
{
  ...
  public function profile($slug, $media = null)
  {
    ...
    // Stripe Key
      ...
      $subscriptionsActive = $user->mySubscriptions()
       ...
       $categoriesids1 = explode(",",$user->categories_id);
       $categoriesids = "";
        if(count($categoriesids1) > 1){
			$categoriesids = DB::table('categories')
			   ->whereIn('id', $categoriesids1 )
			   ->get();
			   
		}else{
			
			$categoriesids = DB::table('categories')
			   ->where('id', $categoriesids1 )
			   ->get();
		}
		
		/*$videocategoriesids = DB::table('updates')
		->distinct()
			   ->where('user_id', $user->id )
			   ->where('video_categories', '<>', null )
			   ->where('video_categories', '<>','' )
			   ->get(['video_categories']);*/
		$useridforme = $user->id;
		$videocategoriesids = DB::table('videocategories')
		->whereIn('id', function($query) use($useridforme)
		{
			$query->select('updates.video_categories')
				  ->from('updates')
				  ->distinct()
				   ->where('user_id', $useridforme )
				   ->where('video_categories', '<>', null )
				   ->where('video_categories', '<>','' )
				  ->whereRaw('updates.video_categories = videocategories.id');
		})
		->get();
      ...
      return view('users.profile',[
          'user' => $user,
            ...
            'videocategoriesids' => $videocategoriesids
        ]);
  ...
  public function postDetail($slug, $id)
  {
    ...
    public function settingsPage()
    {
      ...
      $categories_ids =explode(",",auth()->user()->categories_id); 
      return view('users.edit_my_page', ['genders' => $genders,'categories_ids' => $categories_ids]);
    }
	...
	Validator::extend('letters', function($attribute, $value, $parameters){
      ...
      $user->ADCC_Awards            = strip_tags($this->request->ADCC_Awards);
      $user->IBJJF_Rank            = strip_tags($this->request->IBJJF_Rank);
      ...
      if(isset($this->request->categories_id) && count($this->request->categories_id)){
		  $categories_ids = implode(",",$this->request->categories_id);
	  }
	  
      $user->categories_id   = $categories_ids ?? '';
